...
const staticUrl = process.env.BKPAAS_SUB_PATH ? process.env.BKPAAS_SUB_PATH.substring(0, process.env.BKPAAS_SUB_PATH.length) : ''
...
pageRouter.get('*', async (ctx, next) => {
    ...
    if (HTTP_X_SCRIPT_NAME) {
        renderParams['SITE_URL'] = renderParams['BK_STATIC_URL'] = HTTP_X_SCRIPT_NAME.substring(0, HTTP_X_SCRIPT_NAME.length)
    }
